...
from metacogitor.utils.common import check_cmd_exists
...
async def mermaid_to_file(
    ...
    :type output_file_without_suffix: str
    :param width:
    :type width: int
    :param height:
    :type height: int
    ...
    if engine == "nodejs":
        ...
        if engine == "playwright":
            from metacogitor.utils.mmdc_playwright import mermaid_to_file

            return await mermaid_to_file(
                mermaid_code, output_file_without_suffix, width, height
            )
        elif engine == "pyppeteer":
            from metacogitor.utils.mmdc_pyppeteer import mermaid_to_file

            return await mermaid_to_file(
                mermaid_code, output_file_without_suffix, width, height
            )
        elif engine == "ink":
            from metacogitor.utils.mmdc_ink import mermaid_to_file
...
if __name__ == "__main__":
    ...
    result = loop.run_until_complete(
        mermaid_to_file(MMC1, PROJECT_ROOT / f"{CONFIG.mermaid_engine}/1")
    )
    result = loop.run_until_complete(
        mermaid_to_file(MMC2, PROJECT_ROOT / f"{CONFIG.mermaid_engine}/1")
    )
    loop.close()
