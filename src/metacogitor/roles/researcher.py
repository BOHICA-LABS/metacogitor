...
from metacogitor.actions import CollectLinks, ConductResearch, WebBrowseAndSummarize
from metacogitor.actions.research import get_research_system_text
from metacogitor.const import RESEARCH_PATH
...
class Report(BaseModel):
    ...
    This class is used to define the report.

    Attributes:
        topic (str): The research topic.
        links (dict): The links.
        summaries (list): The summaries.
...
class Researcher(Role):
    """Researcher role, responsible for conducting research.
    ...
    Attributes:
        ...
        language (str): Language of the research system.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        :param language: Language of the research system.
        :type language: str
        """

        super().__init__(name, profile, goal, constraints, **kwargs)
        self._init_actions(
            [CollectLinks(name), WebBrowseAndSummarize(name), ConductResearch(name)]
        )
    ...
    async def _act(self) -> Message:
        ...
        if isinstance(todo, CollectLinks):
            ...
            ret = Message(
                "",
                Report(topic=topic, links=links),
                role=self.profile,
                cause_by=type(todo),
            )
            ...
            ret = Message(
                "",
                Report(topic=topic, summaries=summaries),
                role=self.profile,
                cause_by=type(todo),
            )
        else:
            ...
            content = await self._rc.todo.run(
