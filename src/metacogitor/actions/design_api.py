...
from metacogitor.utils.get_template import get_template
from metacogitor.utils.json_to_markdown import json_to_markdown
from metacogitor.utils.mermaid import mermaid_to_file
...
class WriteDesign(Action):
    ...
    def __init__(self, name, context=None, llm=None):
        """Initializes the WriteDesign action.
    ...
    def recreate_workspace(self, workspace: Path):
        ...
        :param workspace: The workspace to recreate.
        :type workspace: Path
        """
        try:
    ...
    async def _save_prd(self, docs_path, resources_path, context):
        ...
        :param docs_path: The path to the docs folder.
        :type docs_path: Path
        :param resources_path: The path to the resources folder.
        ...
        if (
            context[-1].instruct_content
            and context[-1].instruct_content.dict()["Competitive Quadrant Chart"]
        ):
            quadrant_chart = context[-1].instruct_content.dict()[
                "Competitive Quadrant Chart"
            ]
            await mermaid_to_file(
                quadrant_chart, resources_path / "competitive_analysis"
            )
    ...
    async def _save_system_design(self, docs_path, resources_path, system_design):
        ...
        :param docs_path: The path to the docs folder.
        :param resources_path: The path to the resources folder.
        :param system_design: The system design to save.
        :return: None
        """
        ...
        system_design_file.write_text(
            (json_to_markdown(system_design.instruct_content.dict()))
        )
    ...
    async def run(self, context, format=CONFIG.prompt_format):
        """Runs the action to write the system design.
        ...
        system_design = await self._aask_v1(
            prompt, "system_design", OUTPUT_MAPPING, format=format
        )
        ...
        setattr(
            ...
            "Python package name",
            system_design.instruct_content.dict()["Python package name"]
            .strip()
            .strip("'")
            .strip('"'),
        )
