...
class PlaywrightWrapper:
    ...
    `pip install metacogitor[playwright]` and download the necessary browser binaries by running the
    command `playwright install` for the first time.

    Attributes:
        browser_type (Literal["chromium", "firefox", "webkit"]): The browser type to use.
        launch_kwargs (dict): The keyword arguments to pass to the browser type's launch method.
        _context_kwargs (dict): The keyword arguments to pass to the browser context's new_page
            method.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initialize the PlaywrightWrapper instance.

        :param browser_type: The browser type to use, defaults to None
        :type browser_type: Literal["chromium", "firefox", "webkit"] | None, optional
...
def _get_install_lock():
    """Get the install lock.

    :return: The install lock.
    :rtype: asyncio.Lock
    """
...
async def _install_browsers(*browsers, **kwargs) -> None:
    ...
    :param browsers: The browsers to install.
    :type browsers: str
    """
...
async def _log_stream(sr, log_func):
    ...
    :type sr: StreamReader
    :param log_func: The log function.
