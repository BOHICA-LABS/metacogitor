"""Search the web using the official Google API."""
# -*- coding: utf-8 -*-
...
class GoogleAPIWrapper(BaseModel):
    """Class representing a Google API wrapper.

    Attributes:
        google_api_key: The Google API key.
        google_cse_id: The Google CSE ID.
        loop: The event loop.
    ...
    def check_google_cse_id(cls, val: str):
        """Check the Google CSE ID.

        :param val: The Google CSE ID.
        :type val: str
        :return: The Google CSE ID.
        :rtype: str
        """
    ...
    async def run(
        ...
        :param as_string: A boolean flag to determine the return type of the results. If True, the function will
        :param focus: Specific information to be focused on from each search result.
        :type focus: list[str] | None
        :return: return a formatted string with the search results. If False, it will return a list of dictionaries
        ...
        future = loop.run_in_executor(
            ...
            self.google_api_client.list(
                q=query, num=max_results, cx=self.google_cse_id
            ).execute,
        )
        try:
            result = await future
